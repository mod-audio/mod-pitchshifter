...
enum {IN, OUT_1, OUT_2, TONE, SCALE, INTERVAL, GAIN_1, GAIN_2, PLUGIN_PORT_COUNT};
...
LV2_Handle PitchShifter::instantiate(const LV2_Descriptor* descriptor, double samplerate, const char* bundle_path, const LV2_Feature* const* features)
{
	...
	plugin->p3 = fftwf_plan_dft_r2c_1d(2*plugin->N, plugin->frames3, plugin->fXa2, FFTW_ESTIMATE);
	plugin->p4 = fftwf_plan_dft_c2r_1d(2*plugin->N, plugin->fXs2, plugin->q2, FFTW_ESTIMATE);
